...
13. Plugin process isolation. **Status: DONE (2026-08-02) — Phase 5 blocker CLEARED.** Two reports: `prototypes/process-isolation-poc/REPORT.md` (job objects) and `prototypes/appcontainer-launch-poc/REPORT.md` (AppContainer launch). Validated unelevated: job memory caps genuinely **enforced** (512 MB allocation refused under a 146 MB cap); per-job accounting readable from the trusted side at **~0.17 ms/query**; a process can be **launched into** an AppContainer; and with zero capabilities granted, a file readable outside the container was **denied inside it** (differential test). Remaining items — restricted tokens, install-directory ACLs for our own binary, positive capability tests, CPU capping — are engineering with known shapes, not open research. See ADR-0004. Everything the permission model promises depends on this working.
