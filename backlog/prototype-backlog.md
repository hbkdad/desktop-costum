...
13. Plugin process isolation (AppContainer/restricted-token proof of concept for untrusted widget/plugin code). **Status: not started — now a BLOCKER for Phase 5, not an optional extra.** [ADR-0004](../docs/architecture/adr/0004-process-and-isolation-model.md) commits to AppContainer + restricted token + job object for sandboxed widget hosts, and none of it is verified. If AppContainer proves impractical for hosting the renderer, the fallback (restricted token without an AppContainer) is weaker and needs its own ADR. Everything the permission model promises depends on this working.
