...
- **Per-monitor wallpaper works** — `IDesktopWallpaper` COM implemented, `SupportsPerMonitor` is now **true**, so the activator no longer has to warn that different wallpapers per monitor are impossible. Serves MVP-primary persona #1 directly. **278 tests.**
